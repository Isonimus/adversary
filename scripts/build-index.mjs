...
export function renderIndex(docs) {
  ...
  out.push('     Regenerate with `node scripts/build-index.mjs`; the pre-commit hook keeps it current. -->');
  out.push('');
...
function main(argv) {
  ...
  if (check) {
    ...
    if (current !== text) {
      console.error(`${path} is out of date — run \`node scripts/build-index.mjs\` and commit the result.`);
      return 1;
    }
    return 0;
  }
